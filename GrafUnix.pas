...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, TeeProcs, TeEngine, Chart, Clipbrd, Printers,
  Series, ExtDlgs, VCF1, Menus, RXCtrls, ComCtrls, ToolWin;
...
S:String;
Clz:Array[0..50] of Extended;
VBz:Array[0..50] of Extended;
VFz:array[0..50] of Extended;
Pz:array[0..50] of Extended;
Si: String;
Soma:Extended;
...
for i:=1 to 50 do if F1Graf.EntryRC[1,i]='' then Break;
r:=i-2;
...
Image1.canvas.Brush.Color:=clWhite;
Image1.canvas.FloodFill(2, 2, clOlive, fsBorder);
Image1.Visible:=False;
Te2.Visible:=False;
Te1.Visible:=False;
Te3.Visible:=False;
...
procedure TGrafForm.BitBtn1Click(Sender: TObject);
begin
if AnaForm.RadioButton7.Checked then Clipboard.Assign(Image1.Picture);
if AnaForm.RadioButton1.Checked then Te1.CopyToClipBoardBitmap;
if AnaForm.RadioButton2.Checked then Te2.CopyToClipBoardBitmap;
if AnaForm.RadioButton6.Checked then Te3.CopyToClipBoardBitmap;
...
begin
if AnaForm.RadioButton7.Checked then Image1.Picture.SaveToFile(SaveDialog1.FileName);
if AnaForm.RadioButton1.Checked then Te1.SaveToBitmapFile(SaveDialog1.FileName);
if AnaForm.RadioButton2.Checked then Te2.SaveToBitmapFile(SaveDialog1.FileName);
if AnaForm.RadioButton6.Checked then Te3.SaveToBitmapFile(SaveDialog1.FileName);
end;
end;
...
if PrintDialog1.Execute then
  begin
  if AnaForm.RadioButton7.Checked then
   begin
   Rect.Left:=0;
   Rect.Top:=0;
 ...
 with Printer do
  ...
  if AnaForm.RadioButton1.Checked then Te1.Print;
  if AnaForm.RadioButton2.Checked then Te2.Print;
  if AnaForm.RadioButton6.Checked then Te3.Print;
  end;
 end;
...
Clw:Array[0..50] of Extended;
VFw:array[0..50] of Extended;
VBw:Array[0..50] of Extended;
begin
try
 Screen.Cursor := CrHourGlass;
 for i:=1 to 50 do
  if F1Graf.EntryRC[1,i]='' then Break;
 r:=i-2;
 for g:=1 to r do begin
...
VFz:array[0..50] of Extended;
Pz:array[0..50] of Extended;
Mid:array[0..50] of Extended;
Std:array[0..50] of Extended;
